...
import AnimejsManager from './AnimejsManager.js';
import type AnimejsController from './AnimejsController.js';
...
type Particle = {
  node: INode<AnimejsManager>;
  launchDelay: number;
  animations: AnimejsController[];
  ...
  const renderer = new RendererMain(
    {
      ...
      animationManager: AnimejsManager,
    },
    'app',
  );

  renderer.on('fpsUpdate', (fps) => {
    console.log(`FPS`, fps);
  });
