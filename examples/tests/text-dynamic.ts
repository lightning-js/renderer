...
/**
 * Dynamic Sentence Text Test
 *
 * Renders 4 columns of rows filling the entire screen (1920x1080).
 * Each row displays a sentence assembled from 3 word sets and updates
 * automatically on an interval.
 *
 * Controls:
 * - Enter: Toggle automatic updates on/off
 * - Space: Manually trigger a single word update
 */
...
export default async function test({ renderer, testRoot }: ExampleSettings) {
  ...
  /** @type {ITextNode[]} */
  const nodes: ITextNode[] = new Array(TOTAL);
  ...
  const updateAll = () => {
    for (let i = 0; i < TOTAL; i++) {
      nodes[i]!.text = randomSentence();
    }
  };

  const startAuto = () => {
    if (autoTimer !== null) return;
    autoTimer = setInterval(updateAll, UPDATE_INTERVAL_MS);
  };
