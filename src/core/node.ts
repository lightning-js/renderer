...
class Node {
    ...
    updateTranslate() {
        mat4.fromTranslation(this._localMatrix, vec3.fromValues(
            this._x, this._y, 1
        ));
        if (this._parent) {
    ...
    updateRotation(v) {
        mat4.rotateX(
            this._localMatrix, this._localMatrix, 0.4
        );
        mat4.rotateY(
            this._localMatrix, this._localMatrix, 0.9
        );
        this.updateTranslate();
    }
    ...
    set src(imageSource) {
        ...
        getTexture({
            type: 'image', id: imageSource, src: imageSource
        }).then((texture) => {
